                ...
                @foreach ($colocations as $colocation)
                    <div class="bg-white p-4 rounded-lg border border-gray-200 flex flex-col justify-between">
                        ...
                        <div>
                            ...
                            <p class="text-gray-500 text-sm line-clamp-2">{{ $colocation->description ?? 'Aucune description fournie.' }}</p>
                        </div>
                        ...
                        <div class="flex items-center justify-between mt-3 mb-2 text-xs text-gray-600">
                            ...
                            <div class="flex -space-x-1">
                                @forelse ($colocation->memberships->take(3) as $membership)
                                    <div class="w-6 h-6 rounded-full bg-indigo-100 text-indigo-700 flex items-center justify-center text-xs border border-white">
                                        {{ substr($membership->user->name ?? '?', 0, 1) }}
                                    </div>
                                @empty
                                    <div class="w-6 h-6 rounded-full bg-gray-200 text-gray-400 flex items-center justify-center text-xs border border-white">
                                        <i class="fa-solid fa-user"></i>
                                    </div>
                                @endforelse
                                @if ($colocation->memberships->count() > 3)
                                    <div class="w-6 h-6 rounded-full bg-gray-200 text-gray-600 flex items-center justify-center text-xs border border-white">
